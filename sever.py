...
class Chatroom_Sever(wx.Frame):

    def __init__(self):
        '''创建窗口'''
        wx.Frame.__init__(self, None, id=102, title='ChatRoom服务器', pos=wx.DefaultPosition, size=(400, 470))
    ...
    # 在文本中显示聊天信息，转发消息给指定客户端（私聊）
    def send_to_client(self, source, client_name, message, data_time):
        send_msg = '(私聊) %s : %s\n时间：%s\n' %(source,message,data_time)
        if client_name in list(self.session_thread_map.keys()):
            client = self.session_thread_map[client_name]
            if client.isOn: #当前客户端是活动
                client.user_socket.send(send_msg.encode('UTF-8'))
        else:
            client = self.session_thread_map[source]
            send_msg = "-----私聊用户 %s 不存在-----\n" %client_name
            client.user_socket.send(send_msg.encode('UTF-8'))
...
class SessionThread(threading.Thread):
    ...
    def run(self): # 会话线程的运行
        ...
        while self.isOn:
            ...
            if data == 'userDisconnect': # 如果客户端点击断开按钮，先发一条消息给服务器：消息的内容我们规定：userDisconnect
                self.isOn = False
                # 有用户离开，需要在聊天室通知其他人
                self.server.show_info_and_send_client("服务器通知","%s离开聊天室！"%self.username,time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
            elif data.startswith('@-'):  # 判断是否为一对一聊天指令
                _, target_client, message = data.split('-', 2)
                # 发送一对一聊天消息给指定客户端
                self.server.send_to_client(self.username, target_client, message, time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
            else:
