...
class Chatroom_Client(wx.Frame):
    def __init__(self,c_name): #c_name:客户端名字
        #调用父类的初始化函数
        wx.Frame.__init__(self,None,id=101,title='%s的客户端界面'%c_name,pos=wx.DefaultPosition,size=(400,650))
        ...
        self.text_pub =  wx.TextCtrl(pl,size=(400,250),style =wx.TE_MULTILINE | wx.TE_READONLY)
        box.Add(self.text_pub,1,wx.ALIGN_CENTER)
        self.text_pri =  wx.TextCtrl(pl,size=(400,150),style =wx.TE_MULTILINE | wx.TE_READONLY)
        box.Add(self.text_pri,1,wx.ALIGN_CENTER)
    ...
    def connect_to_server(self,event):
        ...
        if not self.isConnected:
            server_host_port = ('localhost',10086)
            self.client_socket = socket(AF_INET,SOCK_STREAM)
            self.client_socket.connect(server_host_port)
            # 之前规定，客户端只要连接成功，马上把自己的名字发给服务器
            self.client_socket.send(self.name.encode('UTF-8'))
            t  = threading.Thread(target=self.receive_data)
            self.text_pub.AppendText('多人聊天室: \n \n')
            self.text_pri.AppendText('用户私聊: \n')
            self.text_pri.AppendText('私聊格式如：@-user-message \n \n')
            t.daemon=True # 客户端UI界面如果关闭，当前守护线程也自动关闭
    ...
    def receive_data(self):
        ...
        while self.isConnected:
            ...
            if data.startswith('(私聊) '):
                self.text_pri.AppendText('%s\n'%data)
            else:
                self.text_pub.AppendText('%s\n'%data)
    ...
    def go_out(self,event):
        self.client_socket.send('userDisconnect'.encode('UTF-8'))
        # 客户端主线程也关闭
